...
Tfidf_matrix = mmread('./models/Tfidf.movie_review.mtx').tocsr()
...
print(cosin_sim[0])
print(len(cosin_sim))
recommendations = getRecommendation(cosin_sim)
...
if keyword not in list(embedding_model.wv.index_to_key):
    ...
    sim_word = embedding_model.wv.similarity(keyword, topn=10)
    print(sim_word)
    sentence = [keyword] * 11
    count = 10
